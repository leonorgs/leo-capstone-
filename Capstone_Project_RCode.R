...
levels(data.all$NAICS_18short) <- list(Agri = "Agriculture"
                                       , Fores = "Forestry, Fishing, Min., Oil & Gas"
                                       , Utils = "Utilities"
                                       , Const = "Construction"
                                       ...
                                       , Finan = "Finance, Insurance, Real Est. & Leas."
                                       , ProSc = "Prof., Scientific & Technical Services"
                                       , Mngt = "Management, Admin. & Support"
...
# CROSSTABS NUMERIC AND NOMINAL VARIABLES ####################################


# HOURLY WAGE: SCATTERPLOT ANALYSIS ############################################
# Numeric Variables ############################################################
...
# Default par(mar=c(5.1, 4.1, 4.1, 2.1))
par(mar=c(10.5, 5.5, 1.0, 1.0))
aa <- plot(data.all.19fem$EDUCshort
     , data.all.19fem$HRLYEARN
     , ylim = c(0,115)
     , cex.lab = 1.8
     , cex.axis = 1.8
...
mtext("Hourly Wage", side=2, line=3.6, cex =1.8)
# round(aa$stats,2)
# summary(data.all.19fem$HRLYEARN[data.all.19fem$EDUCshort=="No.HS"])
...
# HOURLY WAGE: CROSSTAB AND BOXPLOT ANALYSIS ###################################
# Nominal Variables ############################################################
# HW v. PROVINCE ###############################################################
...
# WAGES VS NOMINAL VARIABLES (AFTER CORRELATION)
wage_educ <- boxplot(HRLYEARN ~ EDUC + SEX + SURVYEAR, data = data.all, main = "Usual hourly wages")
wage_lfsstat <- boxplot(HRLYEARN ~ LFSSTAT + SURVYEAR + SEX, data = data.all, main = "Usual hourly wages")

# RESEARCH QUESTION #1: VARIABLES TO EXPLAIN HOURLY WAGES #####################
# REGRESSION ANALYSIS #########################################################
...
# paste("Before Transformation:", skewness(model$residuals))
# Prediction
...
# Investigating SCHOOLN Coefficient NA
str(data.all$SCHOOLN)
nrow(train.19male[train.19male$SCHOOLN=="Unknown",])
# Diagnostic Plots
# par(mfrow = c(2, 2))
...
# Investigating SCHOOLN Coefficient NA
str(data.all$SCHOOLN)
nrow(train.09fem[train.09fem$SCHOOLN=="Unknown",])
# Diagnostic Plots
# par(mfrow = c(2, 2))
plot(model, lwd = 6)
# Box Cox Transformation (Selected attributes from Stepwise Regression)
...
# Investigating SCHOOLN Coefficient NA
str(data.all$SCHOOLN)
nrow(train.19fem[train.19fem$SCHOOLN=="Unknown",])
# Diagnostic Plots
# par(mfrow = c(2, 2))
plot(model, lwd = 6)
# Box Cox Transformation (Selected attributes from Stepwise Regression)
...
paste("MAE:", mae)
paste("Percentage of cases with less than 25% error:", diff.25*100)
plot(prediction[,"fit"], test.19fem$HRLYEARN^(-0.15))


# RESEARCH QUESTION #2: HOURLY WAGES COMPARISONS ##############################
# Highest Hourly wages by year and gender, among sectors, industries, occupations

# INDIVIDUAL GROUPS (4) RESULTS ###############################################
# INDUSTRY ####################################################################
# Males 2009
...
# RESEARCH QUESTION 3: GENDER PROPORTIONS ######################################

# Export data.all to CSV to build graph directly on Excel
